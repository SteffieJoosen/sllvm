...
#define rotl(_input_, _ROT_) \
  (((_input_) << (_ROT_)) | ((_input_) >> (8 * sizeof(_input_) - (_ROT_))));

/* Workaround to prevent sancus-sim from "freezing". This workaround is
 * acceptable for evaluating the performance of branch balancing and
 * if-conversion, but prevents the key schedule from being stored in memory.
 */
#if 0 
static uint32_t xm_SB[1024];
static uint32_t xm_RK[40];

define m_SB(_i_) xm_SB[_i_]
define m_RK(_i_) xm_RK[_i]
#else
static volatile uint32_t dummy;

#define m_SB(_i_) dummy
#define m_RK(_i_) dummy
#endif
